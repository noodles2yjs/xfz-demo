...
class NewsListView(View):
    def get(self,request):
        page = int(request.GET.get('p',1))#当前页面
        ...
        # VilewList会将这两个参数自动传入，View 需要手动传入
        ...
        context = {
            ...
            #parse.urlencode--可以把字典数据转化成查询字符串方式：？name=xxx&age=xxx
        ...
        #将get_pagination_data方法返回的数据添加到添加到context中
...
def add_banner(request):
    ...
    if form.is_valid():
        ...
        return restful.result(data={'banner_id': banner.priority})

    else:
        return restful.params_error(message=form.get_errors())
def banner_list(request):
    banners = Banner.objects.all()
    #需要返回前端，以json的形式，所以需要序列化
    seriliazer = BannerSerializer(banners,many=True)
    return restful.result(data=seriliazer.data)
